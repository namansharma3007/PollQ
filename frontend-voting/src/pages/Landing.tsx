...
const Landing = () => {
  ...
  return (
    <div className={`${isDarkMode ? 'bg-black text-white' : 'bg-white text-gray-900'} font-sans transition duration-300`}>
      ...
      <div className={`flex items-center justify-center h-screen ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
        <div className="mx-auto text-center font-medium">
          ...
          <p className="mt-6 text-lg  font-normal">Power Your Decisions with Instant Feedback</p>
          <p className="mt-4 text-lg font-normal">Create live polls in seconds, get real-time responses, and make smarter decisions. </p>
          <button className="mt-4 px-6 py-3 bg-purple-500 text-white rounded-full hover:bg-purple-600">Vote now</button>
        </div>
      </div>

      
      <div className={`py-24 ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'} border-t-2 border-b-2 ${isDarkMode ? 'border-white' : 'border-gray-900'}`}>
      ...
      <div id="about" className={`py-20 ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'} `}>
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-xl">
            ...
            The system is slower and still not completely tamper-proof. We bring the system that is safe, reliable and solves the modern issues like 
            higher reachability of the booth, crowd-free voting, inexpensive, faster results, and others.
          </p>
        </div>
      </div>

      
      <footer className={`py-8 ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'} border-t-2 ${isDarkMode ? 'border-white' : 'border-gray-900'}`}>
        <div className="text-center">
          <div className="text-xl font-bold mb-2">PollQ</div>
          <div className="text-sm">© 2024 | Privacy Policy | Terms of Service | Contact Us</div>
          <div className="mt-4 space-x-6">
            <a href="#facebook" className={`hover:text-yellow-400 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Facebook</a>
            <a href="#twitter" className={`hover:text-yellow-400 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Twitter</a>
            <a href="#linkedin" className={`hover:text-yellow-400 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>LinkedIn</a>
          </div>
        </div>
      </footer>
    </div>
